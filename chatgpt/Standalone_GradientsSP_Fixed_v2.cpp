...
// Stubbed TFT_eSprite class
class TFT_eSprite {
public:
    TFT_eSprite(M5Stub::LcdClass*) {} // Constructor stub
...
// Stubbed Preferences class
class Preferences {
private:
    std::map<std::string, long> storage;
public:
    void begin(const char*, bool) {}
    long getLong(const char* key, long defaultValue) {
        return storage.count(key) ? storage[key] : defaultValue;
    }
